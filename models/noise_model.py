...
class ControlPointBetaNoise(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor, adversarial: bool):
        if not adversarial:
            ...
            # mean_control_points: torch.Tensor = torch.empty((*input.shape[:-2],*self.control_point_shape), device=input.device,dtype=input.dtype).fill_((self.mean_interval[1]+self.mean_interval[0])/2)
...
class NoiseModel(torch.nn.Module):
    ...
    def forward(self, I: torch.Tensor, I_d: torch.Tensor, adversarial: bool) -> torch.Tensor:
        ...
        if not adversarial:
            ...
            # self.control_points_gamma = torch.ones((*I.shape[:-2],*self.grid_size), dtype=I.dtype, device=I.device)
        else:
            # FGSM
